...
public class NamePhoneConsumer {
    ...
    @KafkaListener(topics = "${kafka.name-phone.input-topic}", groupId = "opera-cloud-persistence")
    @Transactional
    public void consume(@Payload String message) {
        try {
            ...
            if ("DELETE".equalsIgnoreCase(record.getOperation())) {
                ...
                log.debug("NAME_PHONE deleted phoneId={}", record.getPhoneId());
            } else {
                repository.save(toEntity(record));
                log.debug("NAME_PHONE upserted phoneId={}", record.getPhoneId());
            }
        } catch (Exception e) {
            log.error("NAME_PHONE consumer failed: {}", e.getMessage(), e);
